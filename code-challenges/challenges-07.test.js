...
const addQuestion = (arr) => arr.map( stuff => `${stuff}?`);
...
const forEachTwoToThe = (arr) => {
  const newPuppy = [];
  arr.forEach( number => newPuppy.push(Math.pow(2, number)));
  return newPuppy;
};
...
const charCode = (arr) => arr.map( (value, index) => arr.joint('').charCodeAt(index));
